...
export const Route = createFileRoute('/workspace/$slug/cycles/new')({
  component: NewCycle,
})
  ...
  const onSubmit = async (values: CreateCycleInput) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: workspaceKeys.cycles(workspace.id),
      })

      navigate({ to: '/workspace/$slug/cycles', params: { slug } })
    } catch (err) {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex justify-end gap-2">
                <Button
                  type="button"
                  variant="ghost"
                  onClick={() => navigate({ to: '/workspace/$slug/cycles', params: { slug } })}
                >
                  Cancel
                </Button>
