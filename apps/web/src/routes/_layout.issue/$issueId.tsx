...
import { ArrowLeft, MessageSquare, Pencil, Check, X, Repeat } from 'lucide-react'
...
import { LabelSelect } from '@/components/issues/label-select'
import {
    useIssue,
    useIssueComments,
    useUpdateIssue,
...
import { useWorkspaceBySlug, useWorkspaceMembers, useWorkspaceLabels } from '@/hooks/use-workspace'
...
function IssueDetail() {
    ...
    const { data: labels = [] } = useWorkspaceLabels(workspace?.id)
    ...
    const isLoading = isLoadingWorkspace || isLoadingIssue

    if (isLoading) {
        return <IssueDetailSkeleton slug={slug} />
    }
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 text-muted-foreground text-sm font-mono">
                            <span>{formatIssueIdentifier(slug, issue.number)}</span>
                            <span>·</span>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            {issue.cycle && (
                                <div className="grid grid-cols-3 items-center gap-2 text-sm">
                                    ...
                                    <div className="col-span-2">
                                        <Link
                                            to="/workspace/$slug/cycles/$cycleId"
                                            params={{ slug, cycleId: issue.cycle.id }}
                                            className="flex items-center gap-2 hover:underline"
                                        >
...
function IssueDetailSkeleton({ slug }: { slug: string }) {
    return (
        <div className="max-w-5xl mx-auto py-6">
            <Link
                to="/workspace/$slug"
